...
#    can2 = Canvas(root, width=800, height=600, bg='white', scrollregion=(0, 0, 8000, 12500))
#    can = Canvas( root,width =800, height = 600, bg = 'white', scrollregion = (0,0,8000,12500))



#    can2.config(width = 230 ,height = 500)
 #   can.config(width = 430 ,height = 500)
...
def fetch2(entries ):
    ...
    childprocess = subprocess.Popen(['Python', 'tf_idf_trigram.py', Textpath])
    childprocess.wait()
...
def NewThreadScrap(en, entry,opweb):
    ...
    if opweb.get() == 'amazon.in':
        ...
        childprocess = subprocess.Popen(['scrapy', 'crawl', 'flipkart'],
                                        ...
                                        , stdin=subprocess.PIPE)
        Textpath = 'flipkart\\flipkart\Extracted_Reviews\\reviews.csv'
        childprocess.communicate(en.get())
        childprocess.wait()
